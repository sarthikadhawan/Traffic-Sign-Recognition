...
img,path = read_image(r"\keep_right_5","png")
img = enhance(img)
...
##    crop_img2 = cv2.bitwise_and(crop_img2,crop_img)
    if(not np.all(output_red==0) and color_ratio_red>0.1 and check_aspect_ratio(aspect_ratio) and check_dimension_within_range(img_height,img_width,crop_img_height,crop_img_width)):
       if len(approx) > 10 or len(approx)==3 or len(approx)==8:
            cv2.fillPoly(img2,[approx],(0,255,255))
            mask1 = cv2.inRange(img2,(0,255,255),(0,255,255))
            img2 = cv2.bitwise_and(img2,img2,mask=mask1)
            crop_img2 = get_crop_image(img2,x,y,w,h,wdth,ht)
            crop_img2[np.where((crop_img2==[0,255,255]).all(axis=2))] = [255,255,255]
            crop_img2 = cv2.bitwise_and(crop_img2,crop_img)
            cv2.imwrite(path+r"\Crop_img_"+str(number)+".png",crop_img2)
            number+=1
